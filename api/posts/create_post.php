    ...
    if (($_GET["action"] == 'create_post')) {
        ...
        if (empty($fileName)) {
            if (!empty($title) && !empty($description)) {
                $insert = "insert into tbl_post(school_id,author_id,author_type,title,allow_like,allow_comment,description,datetime,addedon,status)values('" . $school_id . "','" . $author_id . "','" . $author_type . "','" . $title . "','" . $allow_like . "','" . $allow_comment . "','" . $description . "','" . date("Y-m-d H:i:s", $time) . "','" . $time . "','requested')";
                $run_insert = mysqli_query($db, $insert);
                if ($inserted_id = mysqli_insert_id($db) > 0) {

                    if (!empty($_POST['class_assign_id'])) {
                        $classIds = $_POST['class_assign_id'];
                        $inserClass = multipal_insert_data('tbl_assign_class_post', $inserted_id, $classIds, 'post_id', 'class_id', '');
                    }
                    if (!empty($_POST['section_assign_id'])) {
                        $sectionIds = $_POST['section_assign_id'];
                        $insertSection = multipal_insert_data('tbl_assign_section_post', $inserted_id, $sectionIds, 'post_id', 'section_id', '');
                    }
                    if (!empty($_POST['audience_type'])) {
                        $audienceIds = $_POST['audience_type'];
                        $insertAud = multipal_insert_data('tbl_assign_audience_post', $inserted_id, $audienceIds, 'post_id', 'audience_id', '');
                    }
