...
if (isset($_POST['action']) && $_POST['action'] == 'submit') {
    extract($_POST);
    if ($name == "") {
        $errorMsg[] = "Please enter the Category Name.";
    }
    $name_exist = check_exist($name, 'tbl_category', 'name');
    if ($name_exist == 1) {
    ...
    if (count($errorMsg) == 0) {
        if ($id != "") {
            ...
            $static = "";
            $where = " id='" . $id . "'";
            $update_id = update('tbl_category', $fields, $static, $data, $where);
            if ($update_id) {
            ...
            $fields = array("name");
            $static = "";
            $insert_id = insert('tbl_category', $fields, $static, $data, $floor, "");
            if ($insert_id > 0) {
