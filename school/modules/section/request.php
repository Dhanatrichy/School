...
if (isset($_POST['action']) && $_POST['action'] == 'submit') {
    extract($_POST);
    if ($name == "") {
        $errorMsg[] = "Please Enter the Section Name.";
    }
    $name_exist = check_exist($name, 'tbl_section', 'name');
    if ($name_exist == 1) {
        $errorMsg[] = "Section Name Already Exist.";
    }
    if (count($errorMsg) > 0) {
    ...
    if (count($errorMsg) == 0) {
        if ($id != "") {
            ...
            $static = "";
            $where = " id='" . $id . "'";
            $update_id = update('tbl_section', $fields, $static, $data, $where);
            if ($update_id) {
            ...
            $insert_id = insert('tbl_section', $fields, $static, $data, $floor, "");
            if ($insert_id > 0) {
