...
if (isset($_POST['action']) && $_POST['action'] == 'submit') {
    extract($_POST);
    if ($name == "") {
        $errorMsg[] = "Please Enter the Level.";
    }
    $name_exist = check_exist($name, 'tbl_class', 'name');
    if ($name_exist == 1) {
        $errorMsg[] = "This Level Already Exist.";
    }
    if (count($errorMsg) > 0) {
    ...
    if (count($errorMsg) == 0) {
        if ($id != "") {
            ...
            $static = "";
            $where = " id='" . $id . "'";
            $update_id = update('tbl_class', $fields, $static, $data, $where);
            if ($update_id) {
            ...
            $insert_id = insert('tbl_class', $fields, $static, $data, $floor, "");
            if ($insert_id > 0) {
