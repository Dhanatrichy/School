...
if (isset($_POST['action']) && $_POST['action'] == 'submit') {
    ...
    if($id==''){
        $email_exist = check_exist($email, 'tbl_teacher', 'email');
        if ($email_exist == 1) {
...
//    }
    ...
    if ($zipcode == 0) {
        $errorMsg[] = "<br />Please Enter Zipcode.";
    }
    ...
    if (count($errorMsg) == 0) {
        if ($id != "") {
            $data = $_POST;
            $fields = array("name","email","mobile_no","address","state_id","city_id","zipcode","qualification","experience");
...
if (isset($_POST['action']) && $_POST['action'] == 'view_detail') {
    ...
    while ($row = mysqli_fetch_assoc($run)) {
        ...
        $list[] = array('id' => $row['id'], 'school_name' => $_SESSION['school_name'], 'name' => $row['name'], 'email' => $row['email'], 'city' => $city_name, 'state' => $state_name, 'mobile_no' => $row['mobile_no'], 'qualification' => $row['qualification'], 'experience' => $row['experience'], 'subjects' => $subject_names, 'dated' => $row['datetime'], 'status' => ucfirst($row['status']), 'address' => $address);
    }
