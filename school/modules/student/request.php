...
if (isset($_POST['action']) && $_POST['action'] == 'view_detail') {
    ...
    while ($row = mysqli_fetch_assoc($run)) {
        ...
        $list[] = array('id' => $row['id'], 'school_name' => $_SESSION['school_name'], 'name' => $row['name'], 'email' => $row['email'], 'mobile_no' => $row['contact'], 'father_name' => $row['father_name'], 'mother_name' => $row['mother_name'], 'subjects' => $subject_names, 'dated' => $row['datetime'], 'status' => ucfirst($row['status']), 'address' => $address, 'class' => $class.' - '.$section, 'dob' => $row['dob']);
    }
