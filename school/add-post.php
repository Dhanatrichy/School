...
if ($_GET['action'] == "edit" && isset($_GET['id'])) {
    ...
    if($max3['author_type']=='school'){
        $image  =   'uploads/posts/'.$max3['image'];
    }else if($max3['author_type']=='teacher'){
        $image  =   '../teacher/uploads/posts/'.$max3['image'];
    }
}
?>
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($errorMsg == '') {
                ...
                if ($id != "") {
                    ...
                    $data = $_POST;
                    $fields = array("title", "description", "allow_like", "allow_comment");
                    $static = ", image='" . $single_image . "', addedon='".$time."', datetime='".date("Y-m-d H:i:s", $time)."', school_id='".$_SESSION['school_id']."', author_id='".$_SESSION['school_id']."', author_type='school'";
                    ...
                    if ($insert_id > 0) {
                        ...
                        if (!empty($_POST['class_assign_id'])) {
                            ...
                            $inserClass = multipal_insert_data('tbl_assign_class_post', $insert_id, $classIds, 'post_id', 'class_id', '');
                        ...
                        if (!empty($_POST['section_assign_id'])) {
                            ...
                            $sectionIds = $_POST['section_assign_id'];
                            $insertSection = multipal_insert_data('tbl_assign_section_post', $insert_id, $sectionIds, 'post_id', 'section_id', '');
                        ...
                        if (!empty($_POST['audience_type'])) {
                            ...
                            $audienceIds = $_POST['audience_type'];
                            $insertAud = multipal_insert_data('tbl_assign_audience_post', $insert_id, $audienceIds, 'post_id', 'audience_id', '');
