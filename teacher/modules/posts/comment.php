...
if (isset($_POST['action']) && $_POST['action'] == 'post_comment') {
    ...
    $insert = "insert into tbl_post_comment(post_id,user_type,user_id,comment,datetime,addedon)values('" . $post_id . "','" . $_SESSION['user_type'] . "','" . $_SESSION['teacher_id'] . "','" . $comment_box . "','" . date("Y-m-d H:i:s", $time) . "','" . $time . "')";
    $run_insert = mysqli_query($db, $insert);
    if (mysqli_insert_id($db) > 0) {
        
        // Gets comments id regarding User Type
        $query_get_comments_id = "select DISTINCT(post_comment_id) from tbl_post_comment where post_id='" . $post_id . "' and user_type='".$_SESSION['user_type']."' and user_id='".$_SESSION['teacher_id']."'";
